...
**涉及模块/文件**：`src/renderer/src/components/SettingsView.vue` 提供工作区内供应商配置、目录搜索、连接测试、模型与请求头编辑、空状态和运行锁定提示；`stores/agent.ts` 管理供应商目录与激活模型状态；`src/shared/contracts.ts` 定义供应商档案、草稿、协议、连接结果和 IPC；`src/preload/index.ts` 暴露固定桥接；`src/main/ipc.ts` 在主进程入口校验输入；`settings-store.ts` 负责 v2 迁移、加密存储和幂等更新；`provider-discovery.ts` 负责模型发现与错误转换；`agent-runtime.ts` 负责 SDK 目录、注册、密钥注入、模型切换和会话释放。
...
**关键决策**：本次只替换 renderer，不修改 preload、IPC、设置版本或 Agent SDK 运行时。由于当前主进程仍会对 `bash`、`edit`、`write` 发起审批，界面保留审批动作并显示“受控执行”，不虚构尚未实现的全自动模式。品牌不使用独立图标，窗口标题、打包名称和工作区统一为“2pi”。

**核心流程**：启动后加载本地设置、供应商目录和会话记录；用户从左侧选择项目、新建或恢复任务，在底部输入区发送要求。流式回复进入中央记录，工具调用显示运行中、完成或失败状态，需要审批时在同一调用链内批准或拒绝。用户可中止任务，并从侧栏进入工作区内设置页管理供应商与模型。

**涉及模块/文件**：`src/renderer/src/App.vue` 负责视图切换和初始化；`components/AppSidebar.vue` 管理项目与会话导航；`TaskWorkspace.vue` 展示任务记录、工具链、审批和错误；`PromptComposer.vue` 管理发送、停止及当前运行合同；`SettingsView.vue` 管理供应商；`stores/agent.ts` 将 IPC 事件归一化为界面状态；`styles.css` 定义视觉令牌和响应规则；`src/renderer/index.html`、`src/main/index.ts` 与 `package.json` 定义正式品牌标题。

**配置项**：不新增持久化配置或环境变量。继续读取 Electron `userData/settings.json` 的 v2 设置和 `userData/sessions` 下的 JSONL 会话；renderer 不直接读取密钥或项目文件。

**变更记录**：2026-07-25 13:40 CST，正式替换为 Codex 风格任务工作台，拆分 renderer 模块，新增工具状态链、工作区内设置视图、窄窗口图标栏和纯文字 2pi 品牌。
